...
def configure_logging(level=None) -> None:
    ...
    What gets logged at DEBUG (see rules_engine/db_ops.py's module docstring
    ...
    at (db/connection_factory.py), and a "run_rule_group starting" line per
    ...
    where <base> is GRE_LOG_FILE (default "rules_engine", a ".log" suffix in an
    ...
    interleave into one "rules_engine.log" file, which meant grepping
    ...
    if not logging.getLogger().handlers:
        ...
        base_name = _base_log_name("rules_engine")
        try:
        ...
        logging.getLogger(__name__).info(
            "rules_engine logging to %s (level=%s, one file per run, retention=%s).",
