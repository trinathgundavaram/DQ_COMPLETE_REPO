-- ============================================================
-- The existing COMO weekly UM sample (dq_sampling_config.config_id=1,
-- see config/seed/01_setup.sql), re-expressed as gre_sampling_config /
-- gre_sampling_strata / gre_sampling_mix rows.
--
-- This is the DONE CRITERIA regression fixture: tests/test_gre_sampling.py
-- runs this exact config (via the equivalent in-memory rows, same values)
-- and core/stratified_sampling.py's dq_sampling_config=1 side by side
-- against the same fixture universe and asserts matching per-bucket
-- selected counts -- proving the redesign doesn't regress the one real
-- case that already exists, not just "should work" on the recursive
...
-- -- zero changes to gre/sampling.py.
