...
--     parallel run-log table just for sampling") kept as a single shared
--     table -- not split into two independent audit tables -- when this
--     repo was reorganized into separate rules_engine/ and sampling/
--     folders, so it lives here in shared/ rather than being duplicated
--     or force-fit under either package.
...
--     rule_id/rule_group/batch_id are NULL for a sampling-run error row.
--
