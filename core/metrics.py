...
core/metrics.py
---------------
Post-run metrics calculation and dq_metrics_summary upsert.

Fix #3 (v2): MERGE race condition — when two concurrent runs for the same
(project, process, run_type, run_month, batch_id, dataset_id) both find no
matching row, both attempt INSERT and one gets a unique-index violation.
The fix: catch the duplicate-key error and fall back to a pure UPDATE so the
totals are still correctly accumulated.
"""
