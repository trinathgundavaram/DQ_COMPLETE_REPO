"""
gre/sampling.py tests. Same DuckDB-fixture convention as the rest of the
gre/ test suite.

The centerpiece is test_um_regression_matches_dq_stratified_sampling:
it builds the EXACT SAME fixture universe as
tests/test_sampling_and_reporting.py (same random.seed(42), same category/
area distributions) and runs it through BOTH core/stratified_sampling.py
(the proven two-level implementation) and gre/sampling.py (the new
N-level generalization), using equivalent config values, then asserts the
per-bucket breakdowns actually match -- not "should work in theory."
"""
import sys, os
...
# ── UM regression check against core/stratified_sampling.py ─────────────

def test_um_regression_matches_dq_stratified_sampling():
    ...
    dq_config = {
        "config_id": 1, "project_name": "ANY_PROJECT", "process_name": "WEEKLY_REVIEW_SAMPLE",
        "sample_name": "weekly review sample", "connection_name": "duckdb_test",
        "universe_table": "case_universe", "key_columns": "case_id", "scope_column": "pull_date",
        "target_volume": 150, "determination_column": "category",
        "determination_mix_json": json.dumps({"Denied": 0.80, "Withdrawn": 0.10, "Dismissed": 0.02, "Approved": 0.08}),
        "functional_area_column": "area",
        "functional_area_mix_json": json.dumps({"Area A": 0.13, "Area B": 0.08}),
        "exclusion_sql": "auto_closed = 1", "priority_rank_sql": "revision DESC",
    }
    dq_run = {"run_id": "DQ_RUN", "start_date": "2026-08-01", "end_date": "2026-08-01"}
    dq_result = run_stratified_sampling(cf, conn, dq_config, dq_run, "main")
    ...
    assert gre_result["candidates"] == dq_result["candidates"]
    ...
    assert gre_result["by_stratum"] == dq_result["by_stratum"]
    assert gre_result["selected"] == dq_result["selected"]
